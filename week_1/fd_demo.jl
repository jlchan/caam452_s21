...
# define forward, backwards, and centered differences
...
hvec = 2.0 .^(-(0:8))
err = ntuple(x->zeros(size(hvec)),4)
...
plot!(hvec,err[2],marker=:dot,label="Backwards difference")
plot!(hvec,err[3],marker=:dot,label="Centered difference")
