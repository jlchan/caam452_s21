...
"This code solves the 2D heat equation with zero Dirichlet boundary conditions using
a finite difference method. It uses Cholesky factorization to speed up repeated matrix solves." 
...
Bsparse = (I + .5*dt*A)
Blu = lu(I + .5*dt*A)
Bchol = cholesky(I + .5*dt*A)

plot()
@gif for i = 1:Nsteps
    t = i*dt
    Favg = .5*(F(t) + F(t+dt))
    u .= Bchol\(u + .5*dt*(Favg-A*u))
    if i%interval==0
